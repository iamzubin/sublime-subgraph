...
export function handlePoolCreated(event: PoolCreated): void {
  NewPool.createWithContext(event.params.pool, context);
  PoolToken.createWithContext(event.params.poolToken, context);

  let poolContract = PoolContract.bind(event.params.pool);
  ...
  pool.borrowAssetDecimal = BigInt.fromI32(18);
  ...
  pool.collateralAssetDecimal = BigInt.fromI32(18);
...
export function handleOwnershipTransferred(event: OwnershipTransferred): void {
  ...
  let poolFactoryContract = PoolFactory.bind(event.address);

  gPoolDetails.admin = event.params.newOwner.toHexString();
  gPoolDetails.collectionPeriod = poolFactoryContract.collectionPeriod();
  gPoolDetails.matchCollateralRatioInterval =
    poolFactoryContract.matchCollateralRatioInterval();
  gPoolDetails.marginCallDuration = poolFactoryContract.marginCallDuration();
  gPoolDetails.collateralVolatilityThreshold =
    poolFactoryContract.collateralVolatilityThreshold();
  gPoolDetails.gracePeriodPenaltyFraction =
    poolFactoryContract.gracePeriodPenaltyFraction();
  gPoolDetails.liquidatorRewardFraction =
    poolFactoryContract.liquidatorRewardFraction();
