import { SavingAccount, SavingsDeposit } from "../../generated/schema";
import {
  Deposited,
  StrategySwitched,
  Withdrawn,
...
export function handleDeposited(event: Deposited): void {
  ...
  savingDeposit.liquidityShare = savingAccountContract.userLockedBalance(
    event.params.user,
    event.params.asset,
    event.params.strategy
  );

  savingDeposit.amount = savingDeposit.amount.plus(event.params.amount);
...
export function handleStrategySwitched(event: StrategySwitched): void {
  let depositIdFrom =
    ...
    event.params.asset.toHexString() +
    event.params.currentStrategy.toHexString();
  let depositIdTo =
    event.params.user.toHexString() +
    event.params.asset.toHexString() +
    event.params.newStrategy.toHexString();

  let savingDepositFrom = SavingsDeposit.load(depositIdFrom);
  let savingDepositTo = SavingsDeposit.load(depositIdTo);

  savingDepositFrom.liquidityShare = savingAccountContract.userLockedBalance(
    event.params.user,
    event.params.asset,
    event.params.currentStrategy
  );
  savingDepositTo.liquidityShare = savingAccountContract.userLockedBalance(
    event.params.user,
    event.params.asset,
...
export function handleWithdrawn(event: Withdrawn): void {
  ...
  let temp = savingAccountContract.userLockedBalance(
    event.params.from,
    event.params.token,
    event.params.strategy
  );
...
export function handleWithdrawnAll(event: WithdrawnAll): void {
  let depositId =
    event.params.user.toHexString() + event.params.asset.toHexString();
