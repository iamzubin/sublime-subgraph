...
let creditLinesContract = creditLineContract.bind(CREDIT_LINE_ADDRESS);
...
export function handleCreditLineRequestedToLender(
  ...
  if (creditLine == null) {
    ...
    let CreditLineVars = creditLinesContract.try_creditLineInfo(
      event.params.creditLineHash
    ).value;
...
export function handleCreditLineRequestedToBorrower(
  ...
  if (creditLine == null) {
    creditLine = new CreditLine(creditLineHash);
    creditLine.CreditLineHash = event.params.creditLineHash;
    let CreditLineVars = creditLinesContract.creditLineInfo(
      event.params.creditLineHash
    );
    creditLine.lender = lender;
    creditLine.Borrower = borrower;
...
export function handleCreditLineClosed(event: CreditLineClosed): void {
  ...
  let creditLine = CreditLine.load(creditLineHash);
  if (creditLine != null) {
    creditLine.creditLineStatus = getCreditLineStatus(3);
    let CreditLineUsageVars = creditLinesContract.creditLineUsage(
      event.params.creditLineHash
    );
